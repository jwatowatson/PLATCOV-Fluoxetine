...
library(dplyr)
#######################################################################
intervention = c('Fluoxetine', "Fluoxetine_meta") # prefix of analysis file
ref_arm = 'No study drug'
#######################################################################
data_list <- list()
for(i in 1:length(intervention)){

f_name = paste0(intervention[i],'_analysis.csv')
...
save(data_list,
     model_settings,
     all_priors,
     file = "Rout/model_settings_Fluoxitine_new.RData")
#######################################################################