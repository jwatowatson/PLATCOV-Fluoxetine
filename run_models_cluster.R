...
load(paste0('Rout/model_run_setup_Fluoxetine.RData'))
model_settings_temp <- model_settings
platcov_dat_analysis_list_temp <- platcov_dat_analysis_list
stan_inputs_temp <- stan_inputs

load(paste0('Rout/model_run_setup_Fluoxetine_meta.RData'))

model_settings <- rbind(model_settings_temp, model_settings)

platcov_dat_analysis_list <- list(platcov_dat_analysis_list_temp, platcov_dat_analysis_list)
names(platcov_dat_analysis_list) <- unique(model_settings$intervention)

stan_inputs <- list(stan_inputs_temp, stan_inputs)
names(stan_inputs) <- unique(model_settings$intervention)

intervention <- model_settings$intervention[job_i]

writeLines(sprintf('Doing analysis for %s',
                   intervention))
...
stan_input_job = stan_inputs[[intervention]][[model_settings$dataset[job_i]]]
  ...
  save(out, file = paste0('Rout/model_fits_',job_i, "_", intervention, '.RData'))# save output
