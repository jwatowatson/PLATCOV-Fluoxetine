...
for(i in 1:length(levels(dataplot_test$Timepoint_ID))){
  ...
  out = capture.output(cen2means(dataplot_test2$log10_viral_load, dataplot_test2$censor2, dataplot_test2$Trt, LOG = F))
  res1 <- str_extract_all(out[3], "[0-9]+\\.[0-9]+") %>% unlist() %>% as.numeric()
  res2 <- str_extract_all(out[4], "[0-9]+\\.[0-9]+") %>% unlist() %>% as.numeric()
  
  summary_res <- data.frame("Timepoint_ID" = levels(dataplot_test$Timepoint_ID)[i],
                        "mean_Flx" = res1[1], "mean_NSD" = res1[2],
                        "p_val" = res2[2])
...
plot_ecdf <- ggplot() +
  ...
  theme(axis.title = element_text(face = "bold")) +
  geom_vline(data = Summary, aes(xintercept = mean_Flx), col = "#26828EFF", linetype = "dashed") +
  geom_vline(data = Summary, aes(xintercept = mean_NSD), col = "#6DCD59FF", linetype = "dashed") +
  geom_text(data = Summary, mapping = aes(label = lab_p_val), x = 5, y = 0, hjust = 0, vjust = 0, size = 3) +
  geom_text(data = Summary, mapping = aes(label = lab_flx), x = 5, y = 0.1, hjust = 0, vjust = 0, size = 3) +
  geom_text(data = Summary, mapping = aes(label = lab_nsd), x = 5, y = 0.2, hjust = 0, vjust = 0, size = 3) 
