...
platcov_dat_analysis_list[[2]] = 
  platcov_dat %>% ungroup() %>%
  filter(Time <= 5.5, mITT) %>%
  arrange(log10_viral_load==log10_cens_vl) %>%
  mutate(Variant = as.factor(Variant),
         Site = as.factor(Site),
         RnaseP_scaled = scale(40 - CT_RNaseP,scale = F),
...
writeLines(sprintf('Analysis dataset contains %s patients and %s datapoints (%s above LLOD, %s%%)',
                   length(unique(platcov_dat_analysis_list[[1]]$ID)),
                   nrow(platcov_dat_analysis_list[[1]]),
                   sum(platcov_dat_analysis_list[[1]]$CT_NS<40),
                   round(100*mean(platcov_dat_analysis_list[[1]]$CT_NS<40))))
