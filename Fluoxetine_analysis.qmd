...
G1 <- ggplot(effect_fluoxetine, 
  ...
  scale_y_continuous(labels = formatter, limits = c(0.9, 2.8), expand = c(0,0)) +
...
G2 <- ggplot() +
  geom_jitter(data = dataplot, aes(x = Timepoint_ID_num, y = log10_viral_load, col = Trt), 
              ...
              width = 0.15) +
  scale_fill_manual(values = c("#005AB5", "#DC3220", "#F99417"), name = "") +
  ...
  scale_color_manual(values = c("#005AB5", "#DC3220", "#F99417"), name = "") +
  theme_bw() +
...
t_half$Trt <- factor(t_half$Trt, levels = c("Fluoxetine", "Nirmatrelvir", "No study drug"))
