---
title: "Fluoxetine symtomp analysis PLATCOV"
author: "James Watson"
format: pdf
editor: visual
---
...
for(id in 1:length(unique(sympt_dat_for_plot$Label2))){
  ...
  if(all(!sympt_dat_for_plot$sq_yn[ind])){ # never symtomps
    sympt_dat_for_plot$clearance_time[ind]=0
    never_list <- c(never_list, id)
  } else if(all(sympt_dat_for_plot$sq_yn[ind])){ # always symtomps
    writeLines(sprintf('all symtomps for %s with %s FUP points',id,sum(ind)))
...
```{r}
sympt_dat_for_plot2 <- sympt_dat_for_plot#[sympt_dat_for_plot$symtomps_Baseline == 1, ]
...
cols <- trt_colors[c("Fluoxetine","Nirmatrelvir + Ritonavir", "No study drug")]
...
survplot <- survminer::ggsurvplot(fit,pval=T,
                                  ...
                                  palette = cols,
                                  ggtheme = theme_bw(),
                                  legend.labs = c("Fluoxetine", "Nirmatrelvir", "No study drug"),
                                  break.time.by = 4,
                                  pval.size = 4,
                                  pval.coord = c(25,0.9),
                                  size = 1.2,
...
survplot$plot <- survplot$plot + 
  ...
  xlab("symtomps clearance time (days)") +
  ylab("Proportion of patients with symtomps") +
...
```{r}
png("Plots/Symptom_surv_plot.png", width = 6, height = 6, units = "in", res = 350)
survplot
dev.off()
```
